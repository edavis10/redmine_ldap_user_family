...
* Send to alternative email addresses for issue emails (optional)
...
4. If you try to login as a student, it should also create the parent user account.  Logging in as the parent will create the student account.

When a parent or student opens an issue, both the parent and student will be included as recipients.

If you define a custom field for an Alternative email (see doc/config.png), then that email address will be used for issue notifications instead of their user email address.
